#不同产品的测量点位对应的Vision工程:
product_name_to_vision_prefix = {'081D_FrSUS': 'test_visionTo', '081D_FrSUS_1': 'test_visionTo_1'}
tcp_client_dict = {'192.168.3.240': 'task_computer', '192.168.3.145': 'robotLeft'}

product_vision_index = [{"product": "FrSus", "index": [{"name": "part_of_measure", "value": [{"step": 1, "version": "datum1_radius"},
                                                                                      {"step": 2, "version": "datum2_radius"},
                                                                                      {"step": 3, "version": "datum3_radius"},
                                                                                      {"step": 4, "version": "datumPosesAndPoses123Calculation"},
                                                                                      {"step": 5, "version": "孔1"},
                                                                                      {"step": 6, "version": "孔2"},
                                                                                      {"step": 7, "version": "孔3"},
                                                                                      {"step": 8, "version": "datum4_radius_position"},
                                                                                      {"step": 9, "version": "5-1"},
                                                                                      {"step": 10, "version": "5-2"},
                                                                                      {"step": 11, "version": "孔7-1"},
                                                                                      {"step": 12, "version": "孔7-2"},
                                                                                      {"step": 13, "version": "孔7-3"},
                                                                                      {"step": 14, "version": "孔7-4"},
                                                                                      {"step": 15, "version": "孔6-1上"},
                                                                                      {"step": 16, "version": "孔6-1下"},
                                                                                      {"step": 17, "version": "孔6-2上"},
                                                                                      {"step": 18, "version": "孔6-2下"}
                                                                                 ]
                                                             }
                                                            ]
                         }
...
measurement_robot_positions = {"FrSus": {"datum1_radius": [0.8607449340820312,-0.3306180419921875,0.820971954345703,0.008279932963337793,-0.8074776384532386,0.5898387779091646,0.00123376769558227],
...
def get_steps(projdectName,is_part_measure):
    steps = []
    for project in product_vision_index:
        print(project.get('product'))
        if project.get('product') == projdectName:
            if is_part_measure == True:
                for part in project.get('index'):
                    if part.get('name') == 'part_of_measure':
                        for step in part.get('value'):
                            steps.append(step.get('step'))

    return steps
def get_visions(projdectName,is_part_measure,step_name):
    visions = []
    for project in product_vision_index:
        print(project.get('product'))
        if project.get('product') == projdectName:
