...
const QString cPdfViewerUrl = QString("%1/resource/pdfview/viewer.html").arg(QDir::currentPath());
...
QPdfWidget::QPdfWidget(QWidget* pParent) : QWidget(pParent)
{
    ...
    connect(m->pPdfJsBridge, &PdfJsBridge::pdfDocumentloaded, this,
            &QPdfWidget::onRenderPdfFinished);
    m->ready = false;
...
QPdfWidget::~QPdfWidget() { delete m; }
...
void QPdfWidget::closeDocument() { m->pPdfJsBridge->close(); }

void QPdfWidget::setPage(int page)
{
    QString script = QString("PDFViewerApplication.page = %1").arg(page);
...
{
    QVariant res =
        m->pPdfJsBridge->invokeJavaScriptAndWaitForResult("PDFViewerApplication.pagesCount");
    if (res.isValid()) {
        return res.toInt();
    }
    return 0;
...
void QPdfWidget::setToolbarVisible(bool v) { m->pPdfJsBridge->setToolbarVisible(v); }
...
{
    m->pPdfJsBridge->invokeJavaScript(
...
{
    QVariant res = m->pPdfJsBridge->invokeJavaScriptAndWaitForResult(
...
void QPdfWidget::navigateTo(const QString& dest)
{
    ...
    escapedDest.replace("\"", "\\\"");
    QString script =
        QString("PDFViewerApplication.pdfLinkService.navigateTo(\"%1\")").arg(escapedDest);
    m->pPdfJsBridge->invokeJavaScript(script);
}
...
void QPdfWidget::renderPdfData()
{
    ...
    QString script = QString("qpdf_ShowPdfFileBase64('%1')").arg(QString::fromUtf8(m->pdfData));
...
void QPdfWidget::onRenderPdfFinished() { emit pdfDocumentLoaded(); }

void QPdfWidget::setFindFieldText(const QString& text)
{
    // Reset find controller.
    m->pPdfJsBridge->invokeJavaScriptAndWaitForResult(
    ...
    escapedText.replace("\"", "\\\"");
    QString script =
        QString("PDFViewerApplication.findBar.findField.value=\"%1\"").arg(escapedText);
    m->pPdfJsBridge->invokeJavaScript(script);
}
